...
app.post("/request-match", (req, res) => {
  ...
  if (!req.body.resignation_percent) req.body.resignation_percent = 5;
  ...
  if (!req.body.randomcnt) req.body.randomcnt = 0;
  //return res.status(400).send('No randomcnt specified.');

  if (!req.body.number_to_play) req.body.number_to_play = 400;
        ...
        file_promise = new Promise((resolve, reject) => {
          ...
          Promise.all([
            ...
            if (fs.existsSync(temp_file))
              fs.moveSync(temp_file, results.path, {
                overwrite: true
              });
              ...
              db.collection("networks")
                ...
                .then(nets => {
                  if (nets.length == 3) {
                    ...
                    let gameCount = nets[2].game_count;
                    if (gameCount > 0) {
                      db.collection("networks").updateOne(
                        {
                          hash: hash
...
app.get(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    const pug_data = {
      ...
      matches: await dbutils.get_matches_from_db(db, {
...
app.get(
  "/home",
  asyncMiddleware(async (req, res) => {
    const client_list_24hr = await cacheIP24hr.wrap("IP24hr", "5m", () =>
      Promise.resolve(
    ...
    const client_list_1hr = await cacheIP1hr.wrap("IP1hr", "30s", () =>
      Promise.resolve(
        db.collection("games").distinct("ip", {
          _id: {
...
app.get(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    Promise.all([
      ...
      page += "<style>";
      page +=
        "table.networks-table { float: left; margin-right: 40px; margin-bottom: 20px; }\n";
      page += styles;
      ...
      page +=
        ".tooltip .tooltiptextright { visibility: hidden; width: 130px; background-color: black; color: #fff; text-align: center; border-radius: 6px; padding: 5px 0; position: absolute; z-index: 1; top: -5px; left: 110%; }\n";
      page +=
        ' .tooltip .tooltiptextright::after { content: ""; position: absolute; top: 50%; right: 100%; margin-top: -5px; border-width: 5px; border-style: solid; border-color: transparent black transparent transparent; }\n';
      page += " .tooltip:hover .tooltiptextright { visibility: visible; }\n";

      page +=
        ".tooltip .tooltiptextleft { visibility: hidden; width: 130px; background-color: black; color: #fff; text-align: center; border-radius: 6px; padding: 5px 0; position: absolute; z-index: 1; top: -5px; right: 110%; }\n";
      page +=
        ' .tooltip .tooltiptextleft::after { content: ""; position: absolute; top: 50%; left: 100%; margin-top: -5px; border-width: 5px; border-style: solid; border-color: transparent transparent transparent black; }\n';
...
app.get(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    let wIndex = Math.floor(Math.random() * (2 - 0 + 1) + 0);
    task.white_hash = nets[wIndex].hash;
    let end = true;
    while (end) {
      let bIndex = Math.floor(Math.random() * (2 - 0 + 1) + 0);
      if (bIndex != wIndex) {
        task.black_hash = nets[bIndex].hash;
        end = false;
      }
    }
  ...
  db.collection("match_games")
    ...
    .then(({ sgf }) => {
      ...
      switch (req.query.viewer) {
        ...
        case "wgo":
          res.render("wgo", {
            title: "View match " + req.params.hash,
            sgf
          });
          break;
        default:
    ...
    const json = await cachematches.wrap("elograph", "1d", async () => {
      ...
      return Promise.all([
        ...
        .then(dataArray => {
          ...
          cachematches.del("matches", () =>
            console.log("Cleared match table cache.")
          );
          ...
          networks.forEach(item =>
            addNetworkRating(item, ratingsMap.get(item.hash))
          );
...
app.get(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    if (!(start in files)) start = "44";

    const top10 = JSON.parse(
      fs.readFileSync(path.join(__dirname, "static", files[start]))
    );

    return res.render("opening", {
      top10,
      start,
      menu: "opening"
    });
